...
from src.transactions_project.services.security import hash_password

async def create(user: CreateUser, db: AsyncSession):
    db_user = Users(name=user.name, password=hash_password(user.password), email=user.email)
    db.add(db_user)
    await db.commit()
...
async def findUser(email: str, db: AsyncSession):
...
async def update_db(user_id: int, user: UpdateUser, db: AsyncSession):
